...
function TripCard({ trip, onDelete, onShare, isDeleting }: TripCardProps) {
  ...
  const primaryAction = (() => {
    if (isPast || trip.status === 'completed' || trip.status === 'cancelled') {
      return { label: 'View', icon: Eye, href: `/plan/${trip.id}/review` }
    }
    if (trip.status === 'booked') {
      return { label: 'View', icon: Eye, href: `/plan/${trip.id}/review` }
    }
    if (trip.status === 'quoted') {
      return { label: 'Review', icon: ArrowRight, href: `/plan/${trip.id}/review` }
    }
    // draft
    return { label: 'Continue', icon: Pencil, href: `/plan/${trip.id}` }
  })()
      ...
      <div className="absolute bottom-4 right-4 sm:bottom-5 sm:right-5 flex items-center gap-1.5 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100 transition-opacity">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-44">
            ...
            <DropdownMenuItem onClick={() => router.push(`/plan/${trip.id}`)} className="py-3 sm:py-2">
