...
export function SeasonHeroSection({ organization, season, partner, coverNote, eventCount }: SeasonHeroSectionProps) {
  const dateLine = [
    season.start_date ? formatShortDate(season.start_date) : null,
    season.end_date ? formatShortDate(season.end_date) : null,
  ]
    .filter(Boolean)
    .join(' \u2192 ')
    ...
    <section className="bg-gradient-to-b from-kurobeni to-blackberry pt-28 pb-24 px-6 print:bg-white print:pt-8 print:pb-6">
      <div className="max-w-5xl mx-auto">
        ...
        <p className="text-sm text-white/50 mt-1 animate-fade-in-up animation-delay-100 print:text-gray-400">
          Season Recap &middot; {eventCount} event{eventCount !== 1 ? 's' : ''}
        </p>
