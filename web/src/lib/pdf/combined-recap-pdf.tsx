...
const s = StyleSheet.create({
  page: { padding: 40, fontFamily: 'Helvetica', fontSize: 10, color: GRAY_900 },
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', borderBottomWidth: 1, borderBottomColor: GRAY_200, paddingBottom: 12, marginBottom: 20 },
  ...
  partnerNameLg: { fontSize: 22, fontWeight: 'bold', color: DARK },
  subtitle: { fontSize: 10, color: GRAY_500, marginTop: 4 },
  dateRange: { fontSize: 9, color: GRAY_400, marginTop: 6 },
  coverNote: { borderLeftWidth: 3, borderLeftColor: COPPER, backgroundColor: '#faf5f0', paddingHorizontal: 12, paddingVertical: 10, marginTop: 14, borderRadius: 2 },
  coverNoteText: { fontSize: 9, color: GRAY_500, fontStyle: 'italic' },
  statsRow: { flexDirection: 'row', gap: 12, backgroundColor: GRAY_100, borderRadius: 8, padding: 16, marginTop: 18, marginBottom: 18 },
  statBox: { flex: 1 },
  ...
  eventHeader: { fontSize: 12, fontWeight: 'bold', color: DARK, marginTop: 16, marginBottom: 4, borderBottomWidth: 1, borderBottomColor: GRAY_200, paddingBottom: 6 },
  eventSub: { fontSize: 8, color: GRAY_400, marginBottom: 8 },
  catGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  catCard: { width: '31%', borderWidth: 1, borderColor: GRAY_200, borderRadius: 4, padding: 10, marginBottom: 4 },
  ...
  delCard: { borderWidth: 1, borderColor: GRAY_200, borderRadius: 6, padding: 12, marginBottom: 10 },
  ...
  proofPlaceholder: { width: 140, height: 100, backgroundColor: GRAY_100, borderRadius: 4, justifyContent: 'center', alignItems: 'center' },
  ...
  footer: { borderTopWidth: 1, borderTopColor: GRAY_200, paddingTop: 12, marginTop: 24, flexDirection: 'row', justifyContent: 'space-between' },
...
type UsageDeliverable = CombinedRecapData['events'][number]['deliverables'][number]
...
interface CombinedRecapPDFProps {
  data: CombinedRecapData
  shareUrl?: string
}

export function CombinedRecapPDF({ data, shareUrl }: CombinedRecapPDFProps) {
  const { recap, organization, partner, events, stats, eventCount, totalDealValue, dateRange } = data
  const fulfillmentPct = stats.total > 0 ? Math.round((stats.proved / stats.total) * 100) : 0
  const completionPct = stats.total > 0 ? Math.round((stats.completed / stats.total) * 100) : 0

  const dateLine = [
    dateRange.earliest ? formatDate(dateRange.earliest) : null,
    dateRange.latest ? formatDate(dateRange.latest) : null,
  ]
    .filter(Boolean)
    .join(' \u2192 ')
    ...
    <Document title={recap.title} author={organization.name}>
      ...
      <Page size="A4" style={s.page}>
        <View style={s.header}>
          ...
          <Text style={s.headerLabel}>Combined Recap</Text>
        </View>

        <Text style={s.partnerNameLg}>{partner.name}</Text>
        <Text style={s.subtitle}>
          Partnership Overview &middot; {eventCount} event{eventCount !== 1 ? 's' : ''}
          {totalDealValue > 0 ? ` \u00b7 $${totalDealValue.toLocaleString()}` : ''}
        </Text>
        {dateLine && <Text style={s.dateRange}>{dateLine}</Text>}
      ...
      {/* Deliverables pages — grouped by event */}
      <Page size="A4" style={s.page}>
        <Text style={s.sectionTitle}>Deliverables &amp; Proof of Performance</Text>

        {events.map((event) => (
          <React.Fragment key={event.id}>
            <Text style={s.eventHeader}>{event.name}</Text>
