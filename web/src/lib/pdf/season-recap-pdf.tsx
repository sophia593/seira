...
import type { SeasonRecapData } from '@/lib/types/database'
...
const s = StyleSheet.create({
  ...
  seasonName: { fontSize: 22, fontWeight: 'bold', color: DARK },
  partnerName: { fontSize: 14, color: GRAY_500, marginTop: 4 },
  seasonDetails: { fontSize: 9, color: GRAY_400, marginTop: 6 },
...
interface SeasonRecapPDFProps {
  data: SeasonRecapData
  shareUrl?: string
}

export function SeasonRecapPDF({ data, shareUrl }: SeasonRecapPDFProps) {
  const { recap, organization, season, partner, events, stats } = data
  const fulfillmentPct = stats.total > 0 ? Math.round((stats.proved / stats.total) * 100) : 0
  const completionPct = stats.total > 0 ? Math.round((stats.completed / stats.total) * 100) : 0

  const dateLine = [
    season.start_date ? formatDate(season.start_date) : null,
    season.end_date ? formatDate(season.end_date) : null,
  ]
    .filter(Boolean)
    .join(' \u2192 ')
    ...
    <Document title={recap.title} author={organization.name}>
      ...
      <Page size="A4" style={s.page}>
        <View style={s.header}>
          ...
          <Text style={s.headerLabel}>Season Recap</Text>
        </View>

        <Text style={s.seasonName}>{season.name}</Text>
        <Text style={s.partnerName}>{partner.name}</Text>
        {dateLine && <Text style={s.seasonDetails}>{dateLine}</Text>}
        <Text style={[s.seasonDetails, { marginTop: 2 }]}>
